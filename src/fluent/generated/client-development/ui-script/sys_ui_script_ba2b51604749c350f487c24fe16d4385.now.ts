...
Record({
    $id: Now.ID['ba2b51604749c350f487c24fe16d4385'],
    table: 'sys_ui_script',
    data: {
        active: 'true',
        global: 'false',
        ignore_in_now_experience: 'false',
        name: 'x_aleen_snguardian.custom.min.js',
        script: `(function(e) {
    var t = {};
        ...
        window.initAddToAppleWallet = function(e) {
            ...
            e.resultResolver = function(result) {
                if (!e.resultDelivered) {
                    ...
                    cDbg('warn', 'SDK-RESULT', 'resultResolver called but result already delivered — ignoring duplicate', {
                        status: result && result.status
                    });
                }
            };
            ...
            var attachMessageListener = function(config, jwtPromise, requestId) {
                ...
                var handler = function(evt) {
                    ...
                    if (evt.data.message === "ready") {
                        ...
                        l = true;
                        jwtPromise.then(function(result) {
                            if (typeof result !== "undefined" && result.status === 200) {
                                cDbg('info', 'SDK-MSG', 'JWS resolved (200) — posting jwtGenerationSuccessful to popup', {
                                    requestId:    requestId,
                                ...
                                cDbg('warn', 'SDK-MSG', 'JWS resolver timed out (408) — posting jwtGenerationUnsuccessful', {
                                    requestId: requestId
                                });
                                evt.source.postMessage({
                                ...
                                cDbg('error', 'SDK-MSG', 'JWS resolved with error status — posting jwtGenerationUnsuccessful', {
                                    requestId: requestId,
                                    status:    result && result.status,
                            ...
                            config.resultResolver({
                                statusMessage: "JWT call failed: " + (err && err.message ? err.message : "unknown error"),
                                status: "500",
                                error: err
                            });
                        });
                    ...
                    if (evt.data.message === "deliverResult") {
                        logDevTiming("initAddToAppleWallet :: deliverResult received", clickStart,
            ...
            t.addEventListener("click", function() {
                ...
                if (body) {
                    ...
                    setTimeout(function() {
                        s.setAttribute(
                            "style",
                            o + "background-color: rgba(0,0,0,0.6); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px)"
                        );
                    }, 400);
                }
            ...
            var runJwtFlow = function(deferred) {
                ...
                (function pollClosed() {
                    if (!n || n.closed) {
                        ...
                        if (!e.resultDelivered) {
                            cDbg('warn', 'SDK-JWT', 'Popup closed by user before provisioning completed — delivering 444 result');
                            e.resultResolver({
                ...
                (e.jwsResolver || e.jwtResolver)()
                    ...
                    .catch(function(err) {
                        clearTimeout(timeoutHandle);
                        logDevTiming("initAddToAppleWallet :: jwsResolver failed", jwtResolverStart,
