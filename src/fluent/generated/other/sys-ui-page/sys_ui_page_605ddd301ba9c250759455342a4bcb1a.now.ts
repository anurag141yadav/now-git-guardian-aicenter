...
UiPage({
    $id: Now.ID['605ddd301ba9c250759455342a4bcb1a'],
    category: 'general',
    endpoint: 'x_aleen_snguardian_Print.do',
...
<g:requires name="x_aleen_snguardian.qrcodejs_min.jsdbx" />
    <g:ui_form>
        ...
        <g:evaluate var="jvar_idenAsset" expression="RP.getWindowProperties().sysparam_idenAsset.toString()" />
        ...
        <j:while test="\${HTML:identityRecord.next()}">
            ...
            <j:set var="jvar_status" value="\${HTML:identityRecord.getDisplayValue('status.code')}"></j:set>
            <j:set var="jvar_type" value="\${HTML:identityRecord.getDisplayValue('type')}"></j:set>
            <j:set var="jvar_email" value="\${HTML:identityRecord.getValue('email')}"></j:set>
        </j:while>
        ...
        <input type="hidden" id="user_email" name="user_email" value="\${HTML:jvar_email}"></input>
    ...
    <footer class="modal-footer flex">
        <g:dialog_buttons_ok_cancel ok_text="Print" ok_title="Print" ok_type="button" ok="return onPrint();" cancel="return onCancel();" cancel_type="button" />
    </footer>
</j:jelly>`,
    ...
    var mapObj = {
        ...
        "##USER_VALID_TO##": date,
        "QR": generateQRCode(),
        "identity.firstName": document.getElementById('user_name').value,
        "photo": document.getElementById('user_photo').value,
        "identity.asset.validTo": date,
        "user_valid_to": date,
        "user_name": document.getElementById('user_name').value,
...
function generateQRCode() {
    ...
    //if valid_badge :: active badge/ success fully provisioned return QR
    //else not
